...
import { AlertTriangle, Briefcase, Check, Copy, Sparkles } from "lucide-react";
import { useState } from "react";

interface JobResponseDisplayProps {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">
              Filtering location, pay range, and employer requirements
            </p>
          </div>
        </div>
      ...
      <div className="mb-6 flex items-center justify-between border-b border-orange-100 pb-4">
        <div className="flex items-center gap-2">
          <span className="inline-flex items-center gap-1.5 rounded-full bg-[#FFF3E8] px-4 py-1.5 text-xs font-bold tracking-wider text-[#8F3E13] uppercase">
            <Briefcase className="h-4 w-4" /> Matched Openings
          </span>
        </div>
        <button
          onClick={handleCopy}
