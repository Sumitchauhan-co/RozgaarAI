...
export const getPaginatedRecruiterApplicationsService = async (
  user: tokenPayload,
  page: number,
  pageSize: number
) => {
  ...
  const [applications, [{ total }]] = await Promise.all([
    db
      .select()
      .from(recruiterApplicationsTable)
      .orderBy(desc(recruiterApplicationsTable.createdAt))
      .limit(pageSize)
      .offset(offset),
    db.select({ total: count() }).from(recruiterApplicationsTable),
  ]);
