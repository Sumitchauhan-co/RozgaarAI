...
export const getPaginatedWorkerApplicationsService = async (
  user: tokenPayload,
  page: number,
  pageSize: number
) => {
  ...
  const [applications, [{ total }]] = await Promise.all([
    db
      .select()
      .from(workerApplicationsTable)
      .orderBy(desc(workerApplicationsTable.createdAt))
      .limit(pageSize)
      .offset(offset),
    db.select({ total: count() }).from(workerApplicationsTable),
  ]);
