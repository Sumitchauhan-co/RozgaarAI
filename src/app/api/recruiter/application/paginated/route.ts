...
import { getPaginatedRecruiterApplicationsService } from "@/app/server/services/recruiterApplication.service";
...
export const GET = authenticate(async (req: NextRequest, context) => {
  try {
    ...
    const result = await getPaginatedRecruiterApplicationsService(
      user,
      page,
      pageSize
    );
