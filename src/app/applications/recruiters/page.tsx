...
export default function RecruiterApplicationsPage() {
  ...
  const { userId, role, recruiterId, setRecruiterId } = useAuthStore();
  ...
  const fetchAllApplications = useCallback(async () => {
    ...
    try {
      const response = await api.get(`/api/recruiter/application/${userId}`);
  ...
  }, [userId, recruiterId, role, router, fetchAllApplications]);

  useEffect(() => {
    const resolveRecruiterProfile = async () => {
      if (!userId || !role || role !== "recruiter" || recruiterId) return;
      ...
      try {
        ...
        console.error("Failed to resolve recruiter profile:", err);
        router.replace("/profile");
      }
    };

    resolveRecruiterProfile();
  }, [userId, recruiterId, role, router, setRecruiterId]);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    if (!userId) {
      setStatusMessage(
        "Please sign in before creating a recruiter application."
      );
      return;
    }
    ...
    try {
      const response = await api.post(
        `/api/recruiter/application/${userId}`,
        payload
      );
      const json = response.data;
      ...
      if (json.success) {
        ...
        setStatusMessage("Recruiter application created successfully.");
        fetchAllApplications();
      }
    } catch (err) {
      console.error("Failed to save recruiter record:", err);
      setStatusMessage("Unable to create the recruiter application right now.");
    } finally {
  ...
  const handleUpdateStatus = async (
    ...
    try {
      const response = await api.patch(
        `/api/recruiter/application/${userId}/${appId}`,
        { status: targetStatus }
      );
      ...
      console.error("Error updating recruiter application state:", err);
    }
  };
  ...
  const handleDelete = async (appId: string) => {
    if (
      !confirm(
        "Are you sure you want to permanently delete this recruiter application?"
      )
    ) {
      return;
    }
    ...
    try {
      ...
      console.error("Error removing recruiter application:", err);
    }
  };
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="mt-1 text-white/80">
                Manage recruiter opportunities and hiring details in one place
              </p>
            </div>
          </div>
          <button
          ...
          {applications.length === 0 ? (
            <div className="rounded-3xl border border-dashed border-gray-300 bg-white py-20 text-center">
              ...
              <p className="font-medium text-gray-500">
                No recruiter application records found yet.
              </p>
            </div>
          ) : (
                ...
                <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                  <div className="flex items-start gap-5">
                    <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-[#F5E7DA]">
                      <Building2 className="text-[#8F3E13]" />
                    </div>
                    <div>
                      ...
                      <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-gray-500">
                        ...
                        {app.salary && (
                          <span className="rounded bg-gray-100 px-2 py-0.5 text-xs font-semibold text-gray-700 uppercase">
                            {app.salary.toLocaleString()} {app.currency} /{" "}
                            {app.payPeriod}
                          </span>
                        )}
                      </div>
